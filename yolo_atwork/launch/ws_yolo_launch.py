...
def generate_launch_description():
    ...
    aruco_params_file = os.path.join(
        ...
        'yolo_atwork',
        'configs',
        'container_aruco_parameters.yaml'
    )

    with open(aruco_params_file, 'r') as file:
        config = yaml.safe_load(file)

    config = config["/container_detection"]["ros__parameters"]
    ...
    aruco_node = Node(
        package='yolo_atwork',
        executable='container_detection.py',
        parameters=[{
    ...
    containerLaunch = IncludeLaunchDescription([
        # Arguments
        marker_size_arg,
